function [z] = ParticleInFiniteBoxWave(c1, c2, c3, x, t)
    ...
    %if(x < L/2)
    %    z = c1 * 0.164633493785601 * 1 * cos((2 * CalculateEnergy(1.28, L)) ^ 0.5 * x) * exp(-1i * CalculateEnergy(1.28, L) * t / 1) + c2 * 0.245331886228279 * 1 * cos((2 * CalculateEnergy(2.54, L)) ^ 0.5 * x) * exp(-1i * CalculateEnergy(2.54, L) * t / 1) + c3 * 1 * cos((2 * CalculateEnergy(3.73, L)) ^ 0.5 * x) * exp(-1i * CalculateEnergy(3.73, L) * t / 1);
    %else
    %    z = c1 * 11.238195873412328 * exp(-(2 * (0.632 - CalculateEnergy(1.28, L)))^0.5 * x) * exp(-1i * CalculateEnergy(1.28, L) * t / 1) + c2 * -30.644182879786840 * exp(-(2 * (0.632 - CalculateEnergy(2.54, L)))^0.5 * x) * exp(-1i * CalculateEnergy(2.54, L) * t / 1) + c3 * exp(-(2 * (0.632 - CalculateEnergy(3.73, L)))^0.5 * x) * exp(-1i * CalculateEnergy(3.73, L) * t / 1);
    %end
